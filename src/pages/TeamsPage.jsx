                  ...
                  {teamsTab==='settings' && isTeamOwner && (
                        <div>
                                        ...
                                        {currentTCs.map(m => {
                                          ...
                                          return (
                                            <div key={m.id} className="card" style={{ padding:'12px 16px', display:'flex', alignItems:'center', gap:12 }}>
                                              ...
                                              <button onClick={async ()=>{
                                                try {
                                                  const {error} = await supabase.from('team_members').update({role:'member'}).eq('user_id',m.id).eq('team_id',profile.team_id).select()
                                                  if (error) { console.error('Remove TC error:', error.message, error); setError(`Failed to remove TC role: ${error.message}`); return }
                                                  const {data:verify} = await supabase.from('team_members').select('role').eq('user_id',m.id).eq('team_id',profile.team_id).single()
                                                  if (verify?.role !== 'member') { setError('TC role removal did not persist — check database permissions.'); return }
                                        ...
                                        {nonTCMembers.map(m => {
                                          ...
                                          return (
                                            <div key={m.id} className="card" style={{ padding:'12px 16px', display:'flex', alignItems:'center', gap:12 }}>
                                              ...
                                              <button onClick={async ()=>{
                                                try {
                                                  const {error, count} = await supabase.from('team_members').update({role:'tc'}).eq('user_id',m.id).eq('team_id',profile.team_id).select()
                                                  if (error) { console.error('Assign TC error:', error.message, error); setError(`Failed to assign TC role: ${error.message}`); return }
                                                  // Verify the update actually changed a row
                                                  const {data:verify} = await supabase.from('team_members').select('role').eq('user_id',m.id).eq('team_id',profile.team_id).single()
                                                  if (verify?.role !== 'tc') { setError('TC role did not persist — check database permissions.'); return }
                          ...
                          <div className="card" style={{ padding: 24 }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
                              <div>
                                <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)' }}>Morning Briefing</div>
                                <div style={{ fontSize: 11, color: 'var(--muted)' }}>AI-generated daily briefing with priorities, pipeline, and calendar (1 credit/day per member)</div>
                              </div>
                              <button
                                onClick={async () => {
                                  const current = teamData?.team_prefs?.ai_tools?.briefing_enabled !== false
                                  const newAiTools = { ...(teamData?.team_prefs?.ai_tools || {}), briefing_enabled: !current }
                                  const newPrefs = { ...(teamData?.team_prefs || {}), ai_tools: newAiTools }
                                  try {
                                    const { error } = await supabase.from('teams').update({ team_prefs: newPrefs }).eq('id', profile.team_id)
                                    if (error) throw error
                                    ...
                                    setError('Failed to update AI settings.')
                                    console.error('toggleBriefing error:', err)
                                  }
                                }}
                                style={{
                                  width: 48, height: 26, borderRadius: 13, cursor: 'pointer', border: 'none',
                                  position: 'relative', flexShrink: 0, transition: 'background .2s',
                                  background: (teamData?.team_prefs?.ai_tools?.briefing_enabled !== false)
                                    ? '#8b5cf6' : 'var(--b2)',
                                }}
                              >
                                <div style={{
                                  ...
                                  background: '#fff', position: 'absolute', top: 3,
                                  transition: 'left .2s',
                                  left: (teamData?.team_prefs?.ai_tools?.briefing_enabled !== false) ? 25 : 3,
                                }} />
                              </button>
                            </div>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
                              <div>
                                <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)', display: 'flex', alignItems: 'center', gap: 8 }}>
                                  CMA Builder
                                  {(teamData?.cma_addon_status === 'active' || teamData?.cma_addon_status === 'trialing') ? (
                                    <span style={{ fontSize: 9, padding: '2px 7px', borderRadius: 4, fontWeight: 700,
                                      background: 'rgba(5,150,105,.1)', color: '#059669', border: '1px solid rgba(5,150,105,.25)',
                                      fontFamily: "'JetBrains Mono',monospace" }}>
                                      ADD-ON ACTIVE
                                ...
                                <div style={{ fontSize: 11, color: 'var(--muted)' }}>AI-powered comp analysis and CMA reports</div>
                              </div>
                              <button
                                onClick={async () => {
                                  const current = teamData?.team_prefs?.ai_tools?.cma_enabled !== false
                                  const newAiTools = { ...(teamData?.team_prefs?.ai_tools || {}), cma_enabled: !current }
                                  const newPrefs = { ...(teamData?.team_prefs || {}), ai_tools: newAiTools }
                                  try {
                                    const { error } = await supabase.from('teams').update({ team_prefs: newPrefs }).eq('id', profile.team_id)
                                    if (error) throw error
                                    ...
                                    setError('Failed to update AI settings.')
                                    console.error('toggleCMA error:', err)
                                  }
                                }}
                                style={{
                                  width: 48, height: 26, borderRadius: 13, cursor: 'pointer', border: 'none',
                                  position: 'relative', flexShrink: 0, transition: 'background .2s',
                                  background: (teamData?.team_prefs?.ai_tools?.cma_enabled !== false)
                                    ? '#d97706' : 'var(--b2)',
                                }}
                              >
                                <div style={{
                                  ...
                                  background: '#fff', position: 'absolute', top: 3,
                                  transition: 'left .2s',
                                  left: (teamData?.team_prefs?.ai_tools?.cma_enabled !== false) ? 25 : 3,
                                }} />
                              </button>
                            </div>
